...
class Chatmessageveiwmodel extends GetxController {
  ...
  //       seenBy: [user.uid],
  //       deletedFor: [],
  //       isDeletedForEveryone: false, messageType: ,
  //     );
  ...
  Future<void> sendMessage({
    required String chatId,
    String? text,
    // REMOVE "List<File>? images" from here. We don't need it.
  }) async {
    final user = _auth.currentUser;
    if (user == null) return;
    ...
    try {
      ...
      // 2️⃣ LOOK AT "selectedImages" (This is the fix!)
      if (selectedImages.isNotEmpty) {
        // Call your upload function (make sure it's named correctly)
        imageUrls = await _uploadimage(
          chatid: chatId,
          messageid: messageId,
          images: selectedImages, // <--- Pass the class variable here
        );
      }
      ...
      // 6️⃣ Update Chatroom Last Message
      await _firestore.collection('chatrooms').doc(chatId).update({
        'lastMessage': messageType == MessageType.image ? '📷 Photo' : text,
        'lastMessageTime': timestamp,
      });

      // 7️⃣ CLEAR THE LIST so they don't get sent again!
